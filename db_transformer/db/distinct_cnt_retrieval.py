...
class DBDistinctCounter(Protocol):
    def __call__(self, conn: Connection, table_name: str, column_name: str, col_type: TypeEngine) -> Optional[int]:
        ...
...
class SimpleDBDistinctCounter(DBDistinctCounter):
    ...
    def __call__(self, conn: Connection, table_name: str, column_name: str, col_type: TypeEngine) -> Optional[int]:
        ...
        if self.force_collation is not None and isinstance(col_type, (String, Text, TEXT, Unicode)):
...
SimpleStringSeriesMapper = Literal[
    'noop',
    'rstrip', 'strip', 'unidecode', 'ci',
    'rstrip_ci', 'strip_ci', 'unidecode_ci',
    'unidecode_rstrip', 'unidecode_strip',
    'unidecode_rstrip_ci', 'unidecode_strip_ci',
]
...
SIMPLE_STRING_SERIES_MAPPERS: OrderedDict[SimpleStringSeriesMapper, SeriesMapper] = OrderedDict([
    ('noop', lambda s: s),
    ...
    ('unidecode_ci', lambda s: s.str.lower().map(_map_safe(lambda s: unidecode(s)))),
    ('unidecode_rstrip', lambda s: s.map(_map_safe(lambda s: unidecode(s))).str.rstrip()),
    ('unidecode_strip', lambda s: s.map(_map_safe(lambda s: unidecode(s))).str.strip()),
    ('unidecode_rstrip_ci', lambda s: s.str.lower().map(_map_safe(lambda s: unidecode(s))).str.rstrip()),
    ('unidecode_strip_ci', lambda s: s.str.lower().map(_map_safe(lambda s: unidecode(s))).str.strip()),
])


def get_string_mapper(mapper: Union[SeriesMapper, SimpleStringSeriesMapper]) -> SeriesMapper:
...
class DBFullDataFetchLocalDistinctCounter(DBDistinctCounter):
    def __init__(self, string_mapper: Union[SeriesMapper, SimpleStringSeriesMapper] = 'noop') -> None:
        super().__init__()
